...
public class ObjParser {
    ...
    public ObjParser(Model3d model3d, String filename) throws FileNotFoundException, IOException {
        this.model3d = model3d;
        model3d.setObjFilename(filename);
        parseObjFile(filename);

        model3d.doneParsingObj(filename);
    }
    ...
    private void processVertex(String line) {
        ...
        Vector3d vec = new Vector3d(values[0], values[1], values[2]);
        
        if($min == null) {
        	$min = new Vector3d(vec);
        }
        else {
        	if(vec.x < $min.x)
        		$min.x = vec.x;
        ...
        model3d.addVertexGeometric(values[0], values[1], values[2]);
    }
    ...
    private void processVertexTexture(String line) {
        ...
        model3d.addVertexTexture(values[0], values[1]);
    }
    ...
    private void processVertexNormal(String line) {
        ...
        model3d.addVertexNormal(values[0], values[1], values[2]);
    }
...
//        System.err.println("Parsed as "+parsedList);
        model3d.addFace(verticeIndexAry);
    }
    ...
    private void processGroupName(String line) {
        ...
        model3d.setCurrentGroupNames(groupnames);
    }
    ...
    private void processObjectName(String line) {
        model3d.addObjectName(line.substring(OBJ_OBJECT_NAME.length()).trim());
    }
    ...
    private void processSmoothingGroup(String line) {
        ...
        model3d.setCurrentSmoothingGroup(groupNumber);
    }
    ...
    private void processUseMaterial(String line) {
        model3d.setCurrentUseMaterial(line.substring(OBJ_USEMTL.length()).trim());
    }
    ...
    private void processMapLib(String line) {
        ...
        model3d.addMapLib(maplibnames);
    }

    private void processUseMap(String line) {
        model3d.setCurrentUseMap(line.substring(OBJ_USEMAP.length()).trim());
    }
...
// 	    }
    ...
    public Vector3d getMax() {
    	System.out.println("Min: " + $max.x + " " + $max.y + " " + $max.z);
    	return $max;
    }
}